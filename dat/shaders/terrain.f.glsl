...
void main() {
	...
	//float y = (texture_normal.y - 0.5) * 2.0;
	//float z = (texture_normal.z - 0.5) * 2.0; 
	vec4 image_normal = vec4( 0.0, 0.0, 1.0, 0.0 );
	float x = (texture_b_normal.x - 0.5) * 2.0;
	float y = (texture_b_normal.y - 0.5) * 2.0;
	float z = (texture_b_normal.z - 0.5) * 2.0; 
 ...
 vec4 normal = cameraSpace_frag_normal;
	...
	vec4 total_light_color = (light_ambient + directional_light_diffuse * diffuse) * ssao * 1.0; 
	...
	vec4 cliff_color_2 = mix( texture2D( tex_d, cliff_texcoord ),
			texture2D( tex_d, cliff_texcoord_b ),
			smoothstep(0.2, 0.8, (abs(frag_normal.z) / ( abs(frag_normal.x) + abs(frag_normal.z)))));
